...
public class ExtendUpperArmByController : MonoBehaviour
{
    ...
    // 上腕セグメントの長さ [m]
    public float upperArmLength = 0.24f;    //0.30 [m]くらい 固定
    // 仮想腕モデルの総重量(TCP送信)
    ...
    {
        // 前腕セグメント長さ計算
        //upperArmLength = Vector3.Distance(RHandController.transform.position, RElbowController.transform.position);
        //Debug.Log("forearm legnth: " + upperArmLength);
    }
    ...
    void Update()
    {
        ...
        if (isStarted)
        {
            ...
            RHand.transform.localPosition = new Vector3(upperArmLength * (extendRatio - 1.0f) + offsetRHand, RHandInitPos.y, RHandInitPos.z);
